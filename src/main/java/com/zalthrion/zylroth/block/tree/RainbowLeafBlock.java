...
public class RainbowLeafBlock extends BlockLeafBase {
	...
	public RainbowLeafBlock() {
		super();
		this.setCreativeTab();
		this.setDefaultState(this.blockState.getBaseState().withProperty(VARIANT, TreeVariantRainbow.RED).withProperty(CHECK_DECAY, Boolean.valueOf(true)).withProperty(DECAYABLE, Boolean.valueOf(true)));
	...
	@Override protected ItemStack createStackedBlock(IBlockState state) {
		return new ItemStack(Item.getItemFromBlock(this), 1, ((TreeVariantRainbow) state.getValue(VARIANT)).getMetadata());
	}
