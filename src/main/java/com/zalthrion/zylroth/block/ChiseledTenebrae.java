...
public class ChiseledTenebrae extends BlockBase {
	...
	@SideOnly(Side.CLIENT) private IIcon frontIcon;
	...
	@SideOnly(Side.CLIENT) public IIcon getIcon(int side, int meta) {
		return side == 1 ? this.topIcon : (side == 0 ? this.topIcon : (meta == 2 && side == 2 ? this.frontIcon : (meta == 3 && side == 5 ? this.frontIcon : (meta == 0 && side == 3 ? this.frontIcon : (meta == 1 && side == 4 ? this.frontIcon : this.blockIcon)))));
	}
	
	@SideOnly(Side.CLIENT) public void registerBlockIcons(IIconRegister register) {
		this.frontIcon = register.registerIcon(Reference.MOD_ID + ":" + name); // Face
		this.topIcon = this.blockIcon = register.registerIcon(Reference.MOD_ID + ":" + "tenebraeBlock"); // Top
		// this.blockIcon = register.registerIcon(Reference.MOD_ID + ":" +
		// "Tenebrae_Block"); //Side
	}
