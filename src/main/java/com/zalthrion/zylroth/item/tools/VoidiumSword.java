...
public class VoidiumSword extends ItemSword implements ZylrothTool {
	public VoidiumSword(ToolMaterial material) {
		...
		this.setNames(this, "voidiumSword");
	}
	
	@Override public boolean isVoidium() {
		return true;
	}
	...
	@Override public ActionResult<ItemStack> onItemRightClick(ItemStack itemStackIn, World worldIn, EntityPlayer playerIn, EnumHand hand) {
		...
		if (!(this.isBroken(itemStackIn)) && !(playerIn.isSneaking()) && !(playerIn.capabilities.isCreativeMode)) {
			
			/** Test-Item Start */
			/* Vec3 vec3 = player.getPosition(1.0F); vec3.yCoord ++; Vec3
			 * lookVec = player.getLook(1.0F); Vec3 aVector =
			 * vec3.addVector(lookVec.xCoord * 50.0D, lookVec.yCoord * 50.0D,
			 * lookVec.zCoord * 50.0D); MovingObjectPosition movingObjPos =
			 * world.rayTraceBlocks(vec3, aVector, true); if (movingObjPos ==
			 * null) { return stack; } else { if (movingObjPos.entityHit
			 * instanceof EntityLivingBase) { return stack; } else {
			 * player.setPositionAndUpdate((double) movingObjPos.blockX,
			 * ((double) (float) movingObjPos.blockY + 1F), (double)
			 * movingObjPos.blockZ); } for (int countparticles = 0;
			 * countparticles <= 100; ++ countparticles) {
			 * world.spawnParticle("portal", (double) player.posX - 0.0F,
			 * (double) player.posY - 0.5F, (double) player.posZ - 0.0F,
			 * (double) ((float) rand.nextFloat() - 0.1F), (double) ((float)
			 * rand.nextFloat() - 0.1F), (double) ((float) rand.nextFloat()) -
			 * 0.1F); world.spawnParticle("portal", (double) player.posX - 0.0F,
			 * (double) player.posY - 0.5F, (double) player.posZ - 0.0F,
			 * (double) ((float) rand.nextFloat() - 1.1F), (double) ((float)
			 * rand.nextFloat() - 0.1F), (double) ((float) rand.nextFloat()) -
			 * 0.1F); world.spawnParticle("portal", (double) player.posX - 0.0F,
			 * (double) player.posY - 0.5F, (double) player.posZ - 0.0F,
			 * (double) ((float) rand.nextFloat() - 0.5F), (double) ((float)
			 * rand.nextFloat() - 0.1F), (double) ((float) rand.nextFloat()) -
			 * 1.1F); } } */
			
			for (int countparticles = 0; countparticles <= 100; ++ countparticles) {
				worldIn.spawnParticle(EnumParticleTypes.PORTAL, playerIn.posX, playerIn.posY - 0.5F, playerIn.posZ, rand.nextFloat() - 0.1F, rand.nextFloat() - 0.1F, rand.nextFloat() - 0.1F);
				worldIn.spawnParticle(EnumParticleTypes.PORTAL, playerIn.posX, playerIn.posY - 0.5F, playerIn.posZ, rand.nextFloat() - 1.1F, rand.nextFloat() - 0.1F, rand.nextFloat() - 0.1F);
				worldIn.spawnParticle(EnumParticleTypes.PORTAL, playerIn.posX, playerIn.posY - 0.5F, playerIn.posZ, rand.nextFloat() - 0.5F, rand.nextFloat() - 0.1F, rand.nextFloat() - 1.1F);
			}
		}
