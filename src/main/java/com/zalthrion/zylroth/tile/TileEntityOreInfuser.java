...
public class TileEntityOreInfuser extends TileEntityBase implements ISidedInventory {
	...
	public void updateEntity() {
		...
		if (!worldObj.isRemote) {// && GuiOreInfuser.craftEnable == true) {
			...
			if (flag != isBurning()) {
				OreInfuserMachine.updateBlockState(this.isBurning(), worldObj, xCoord, yCoord, zCoord);
				flag1 = true;
			}
		}
	...
	private boolean canInfuse() {
		...
		if (slots[ContainerOreInfuser.OUTPUT] == null)
			return true;
		if (!slots[ContainerOreInfuser.OUTPUT].isItemEqual(stack))
			return false;
	...
	public void infuseItem() {
		if (canInfuse()) {
			...
			providedStacks: for (ItemStack provided : providedStacks.values()) {
				...
				if (!OreInfusionRecipeHandler.arrayListContainsItemStack(recipeRequirements, provided))
